Poof = (function() {
  ...
  var poofBreakpoint = function(allBreakpoints) {
  ...
  var cacheNodes = function(nodes) {
    ...
    for( ; i < l; i++ ) {
      ...
      obj = {
        'element' : el,
        'breakpoint' :  el.getAttribute(poof.trigger)
      };
      mqs.push(obj);
    }
    poof.mqs = poofBreakpoint(true);
    return mqs;
  };
  ...
  var testNodes = function() {
    this.forEach(function(node) {
      var mediaMatch = ( !window.matchMedia ) ? noMatchMediaSize : poof.mqs[node.breakpoint] ;
  ...
  document.addEventListener("DOMContentLoaded", function(event) {
    var nodes = cacheNodes( document.querySelectorAll('[' + poof.trigger + ']') );
